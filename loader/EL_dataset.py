...
class ELTestDataset(Dataset):
    def __init__(self, pic_size, category) -> None:
        super(ELTestDataset, self).__init__()
        self.pic_shape = (pic_size, pic_size)
        self.category = category
        self.data_dir = '/root/test/wss/datasets/EL'
    ...
    def get_statistics(self):
        ''' 获得数据集数量信息 '''
        positive_count = len([item for item in self.data_list if 'norm' in item])
    ...
    def get_data_list(self):
        ...
        data_list = []
        mask_list = []
        norm_datas = os.listdir(self.data_dir+'/'+'norm_test')
        norm_datas = [f"{self.data_dir}/norm_test/{item}" for item in norm_datas]
        if self.category != 'elall':
            defect_dir = self.data_dir +'/'+self.category+'_test'
            defect_datas = os.listdir(defect_dir)
            defect_datas = [f"{defect_dir}/{item}" for item in defect_datas]
        else:
            type_dirs = os.listdir(self.data_dir)
            type_dirs = [item for item in type_dirs if 'test' in item and 'norm' not in item]
            defect_datas = []
            for type_dir in type_dirs:
                t_datas = os.listdir(f"{self.data_dir}/{type_dir}")
                t_datas = [f"{self.data_dir}/{type_dir}/{item}" for item in t_datas]
                defect_datas += t_datas
        norm_datas = norm_datas[:len(defect_datas)]
        data_list = defect_datas + norm_datas
    ...
    def __getitem__(self, idx):
        ...
        mask = torch.randint(0,2, [1,self.pic_shape[0],self.pic_shape[1]])
        if defecttype == 'norm_test':
            label = 0
        else:
            label = 1
...
class ELTrainDataset(Dataset):
    ...
    def get_train_data_list(self):
        fdir = os.path.join(self.data_dir, 'norm_train')
        data_list = os.listdir(fdir)
        data_list = [item for item in data_list if 'jpg' in item]
