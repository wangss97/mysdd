...
class SegmentationLosses(object):
    def __init__(self, weight=None, size_average=True, batch_average=True, ignore_index=255, device='cuda'):
...
if __name__ == '__main__':
    ...
    # # model = examp()
    # # torch.save(model.state_dict(),'./tmp/tmp.pth')

    # cpt_dir = './checkpoints/memAE_preConv/cable'
    # checkpoint = torch.load(f'{cpt_dir}/epoch_20.pth', map_location='cpu')

    # # checkpoint = torch.load(f'./tmp/tmp.pth', map_location='cpu')

    # for item, value in checkpoint.items():
    #     if 'memorys.3' in item:
    #         # value = value[:10]

    #         # print(torch.mean(value, dim=1))
