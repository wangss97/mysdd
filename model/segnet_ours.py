...
class Segnet_ours(nn.Module):
    def __init__(self,in_channels=3, out_channels=2, base_channels=64, out_features=False, depth=4):
        super(Segnet_ours, self).__init__()
        base_width = base_channels
        self.encoder = Encoder(in_channels, base_width, depth)
        self.decoder = Decoder(base_width, out_channels=out_channels, depth=depth)
        self.out_features = out_features

    def forward(self, ori_img, rec_img):
        fea_list1 = self.encoder(ori_img)
        fea_list2 = self.encoder(rec_img)
        fea_list = []
        for fea1, fea2 in zip(fea_list1, fea_list2):
            fea_list.append(torch.concat([fea1, fea2], dim=1))
        seg_map = self.decoder(fea_list)
        return seg_map
...
class Decoder(nn.Module):
    def __init__(self, base_width, out_channels=1, depth=6):
        ...
        self.up_b = nn.Sequential(nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                                 nn.Conv2d(base_width * 8 * 2, base_width * 8, kernel_size=3, padding=1),
                                 nn.BatchNorm2d(base_width * 8),
                                 nn.ReLU(inplace=True))
        self.db_b = nn.Sequential(
            nn.Conv2d(base_width*(8+8+8), base_width*8, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*8),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width * 8, base_width * 8, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width * 8),
        ...
        self.db1 = nn.Sequential(
            nn.Conv2d(base_width*(4+8+8), base_width*4, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*4),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width * 4, base_width * 4, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width * 4),
        ...
        self.db2 = nn.Sequential(
            nn.Conv2d(base_width*(2+4+4), base_width*2, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width*2),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width * 2, base_width * 2, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width * 2),
        ...
        self.db3 = nn.Sequential(
            nn.Conv2d(base_width*(1+2+2), base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width, base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
        ...
        self.db4 = nn.Sequential(
            nn.Conv2d(base_width*(1+1+1), base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width, base_width, kernel_size=3, padding=1),
            nn.BatchNorm2d(base_width),
